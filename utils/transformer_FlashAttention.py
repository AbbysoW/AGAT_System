...
class MultiHeadAttention(keras.layers.Layer):
    ...
    def __init__(self, d_model: int, num_heads: int, block_k: int = 64):
        super().__init__()
    ...
    def flash_attention_step(self, q, kj, vj, o, m, l):

        matrix_mult = tf.matmul(q, kj, transpose_b=True)
        d_k = tf.cast(tf.shape(q)[-1], q.dtype)
        scaled_logits = matrix_mult / tf.math.sqrt(d_k)

        max_s = tf.reduce_max(scaled_logits, axis=-1)
        m_new = tf.maximum(m, max_s)

        exp_scale_old = tf.exp(m - m_new)
        exp_scale_new = tf.exp(scaled_logits - m_new[:, :, :, None])

        l = l * exp_scale_old + tf.reduce_sum(exp_scale_new, axis=-1)
        o = o * exp_scale_old[:, :, :, None] + tf.matmul(exp_scale_new, vj)
    ...
    def call(self, q, k, v, mask=None):
        ...
        o = tf.zeros([batch_size, self.num_heads, seq_len, self.depth], dtype=q.dtype)
        m = tf.fill([batch_size, self.num_heads, seq_len], tf.constant(-1e9, q.dtype))
        l = tf.zeros([batch_size, self.num_heads, seq_len], dtype=q.dtype)


        # Apply attention function
        for j in tf.range(0, seq_len, self.block_k):
            kj = k[:, :, j:j+self.block_k, :]
            vj = v[:, :, j:j+self.block_k, :]

            o, m, l = self.flash_attention_step(q, kj, vj, o, m, l)

        scaled_attention = o / l[:, :, :, None]

        if logger_attention.isEnabledFor(logging.DEBUG):
            logger_attention.debug("      Attention shape: %s", 
